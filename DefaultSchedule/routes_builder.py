...
def prompt_airport_code():
    while True:
        code = input("Please enter the ICAO or IATA code of the airport: ")
        if len(code) == 3 and code.isalpha():
            ...
            code_type = "ICAO"
            return code.upper(), code_type
        print("Invalid code. Please enter a valid ICAO (4 letters) or IATA (3 letters) code.") 
...
def build_12hr_windows(end_date: datetime) -> list[tuple[datetime, datetime]]:
    ...
    while current_day <= final_day:
        # AM: 00:00 → 11:59
        am_start = current_day
        am_end = current_day.replace(hour=11, minute=59)

        # PM: 12:00 → 23:59
        pm_start = current_day.replace(hour=12, minute=0)
        pm_end = current_day.replace(hour=23, minute=59)
...
def aggregate_flights_for_csv(all_results: list) -> list[dict]:
    """
    Combines all flights into rows:
    Airline, Airport, Airplane Models, Stand Tags

    One row per airline + airport + stand tag pair.
    ...
    for result in all_results:
        ...
        for flight in departures:
            airline_code = get_airline_code(flight)
            airport_code = get_route_airport_code(flight, "departure")
            aircraft_code = get_aircraft_icao(flight)
            stand_tag = get_stand_tag(flight)

            if airline_code != "UNKNOWN" and airport_code != "UNKNOWN":
                routes[(airline_code, airport_code, stand_tag)].add(aircraft_code)

        for flight in arrivals:
            airline_code = get_airline_code(flight)
            airport_code = get_route_airport_code(flight, "arrival")
            aircraft_code = get_aircraft_icao(flight)
            stand_tag = get_stand_tag(flight)
    ...
    rows = []
    for (airline_code, airport_code, stand_tag), aircraft_set in sorted(routes.items()):
        models = sorted(aircraft_set)

        rows.append({
            "Airline": airline_code,
            "Airport": airport_code,
            "Airplane Models": ":".join(models),
...
def write_csv(rows: list[dict], airport_code: str, end_date: datetime, output_folder: str) -> str:
    """
    Writes the aggregated schedule CSV to disk.
    """
...
def main():
    ...
    rows = aggregate_flights_for_csv(all_results)
    ...
    print(f"Generated {len(rows)} route rows for CSV.")
